...
export const AgentsToolParametersSchema = Schema.Union([
  ...
  Schema.Struct({ action: action("send", "Deliver work to an existing or new child session."), ...SendFields }),
  Schema.Struct({
    action: action("abort", "Stop an active session run."),
    sessionId: optional(SessionReference),
  }),
  Schema.Struct({
