...
export class RuntimeMetadata extends Schema.Class<RuntimeMetadata>(
  "RuntimeMetadata",
)({
  sessionId: SessionId,
...
export const RuntimeRegistryLive = Layer.effect(
  ...
  Effect.gen(function* () {
    const state = yield* SubscriptionRef.make(
      HashMap.empty<SessionId, RuntimeRecord>(),
    );
    const values = (records: HashMap.HashMap<SessionId, RuntimeRecord>) =>
      HashMap.toValues(records);

    return {
      register: Effect.fn("RuntimeRegistry.register")(function* (
        metadata: RuntimeMetadata,
        runtime: object,
      ) {
      ...
      get: Effect.fn("RuntimeRegistry.get")(function* (
        sessionId: SessionId,
      ) {
      ...
      touch: Effect.fn("RuntimeRegistry.touch")(function* (
        sessionId: SessionId,
        lastActivityAt: number,
      ) {
        yield* SubscriptionRef.update(state, (records) => {
      ...
      remove: Effect.fn("RuntimeRegistry.remove")(function* (
        sessionId: SessionId,
      ) {
        yield* SubscriptionRef.update(state, (records) =>
          HashMap.remove(records, sessionId),
        );
      }),
