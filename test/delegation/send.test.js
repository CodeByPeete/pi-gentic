...
import { availableAgentLines } from "../../dist/agents/prompts.js";
import {
  ...
  sessionRunOutcome,
  sessionStatus,
} from "../../dist/delegation/activity.js";
import {
  abortActor,
  contextStillActive,
  deliverCardToCaller,
...
test("foreground sends complete through the managed delegation runtime", async () => {
  ...
  const orchestrator = createOrchestrator({ getAllTools: () => [], sendMessage: () => {} }, effectRuntime);
  orchestrator.load = () => ({});
  orchestrator.resolvePolicy = () => ({ agentsTool: {} });
  orchestrator.resolveTargetSession = async () => target;
  const updates = [];
...
test("background sends survive extension reload and leave the live panel terminal", async () => {
  ...
  const orchestrator = createOrchestrator({ getAllTools: () => [], sendMessage: () => {} }, runtime);
  orchestrator.load = () => ({});
  orchestrator.resolvePolicy = () => ({ agentsTool: {} });
  orchestrator.resolveTargetSession = async () => target;
...
test("orchestrator routes target, status, abort, and policy operations", async () => {
  ...
  const policyOrchestrator = createOrchestrator({ getAllTools: () => [] }, effectRuntime);
...
test("orchestrator invokes registered caller runtimes with structured returns", async () => {
  ...
  try {
    ...
    const runtimeForCallerInvocation = orchestrator.runtimeForCallerInvocation;
    ...
    orchestrator.runtimeForCallerInvocation = runtimeForCallerInvocation;
...
test("orchestrator applies runtime policy and discovers the current session", async () => {
  const runtime = createExtensionRuntime();
  const orchestrator = createOrchestrator({ getAllTools: () => [] }, runtime);
