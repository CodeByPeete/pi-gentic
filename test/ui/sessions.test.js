...
} from "../../dist/ui/cards.js";
import { renderAgentsResult } from "../../dist/ui/card-renderer.js";
import { getLiveCardDetails, setLiveCardDetails } from "../../dist/ui/cards.js";
...
test("live panel gives every active session one compact detailed row", async () => {
  const cards = [
    {
      ...
      activities: [{ type: "tool", name: "read", summary: "src/ui/card-renderer.ts" }],
    },
    {
      cardId: "compact-two",
      kind: "send",
  ...
  try {
    ...
    assert.match(researcherRows[0], /\[ASYNC\].*\(019f9c28-97d3\).*\[read\] src\/ui\/card-renderer\.ts.*idle.*total/);
...
test("duplicate extension instances keep one shared live panel", async () => {
  ...
  const modules = await Promise.all([1, 2].map((id) => import(`../../dist/ui/cards.js?live-panel-${id}=${nonce}`)));
  const calls = [[], []];
...
test("live card updates stay in the visible panel without clearing terminal scrollback", async () => {
  ...
  try {
    ...
    assert.match(writes.join(""), /\[read\].*src\/ui\/card-renderer\.ts/);
